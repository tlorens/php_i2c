...
PHP_ARG_WITH(i2c, for i2c support,
Make sure that the comment is aligned:
[  --with-i2c             Include i2c support])

dnl Otherwise use enable:

dnl PHP_ARG_ENABLE(i2c, whether to enable i2c support,
dnl Make sure that the comment is aligned:
dnl [  --enable-i2c           Enable i2c support])
...
if test "$PHP_I2C" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $I2C_DIR/lib, I2C_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_I2CLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong i2c lib version or lib not found])
  dnl ],[
  dnl   -L$I2C_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(I2C_SHARED_LIBADD)

  PHP_NEW_EXTENSION(i2c, i2c.c, $ext_shared)
fi
